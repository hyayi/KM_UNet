...
def parse_args():
    ...
    # [RESUME] 이어 학습 인자 추가 (경로만 받는 최소 변경)
    parser.add_argument('--resume', type=str, default='',
                        help='checkpoint path (.pth/.pt/.tar). empty = train from scratch')
...
# [RESUME] 체크포인트 저장
def save_ckpt(path, model, optimizer, scheduler, epoch, best_iou, best_dice, config_dict):
    ckpt = {
        'epoch': epoch,
    ...
    torch.save(ckpt, path)  # 공식 가이드: 여러 state를 dict로 묶어 저장 :contentReference[oaicite:1]{index=1}


# [RESUME] 체크포인트 로드 (state_dict/완전 체크포인트 모두 지원, module. 접두어 처리)
def load_ckpt(path, model, optimizer=None, scheduler=None, strict=True):
    ckpt = torch.load(path, map_location='cuda')  # map_location 사용 가능 :contentReference[oaicite:2]{index=2}

    # state_dict만 저장된 경우도 처리
    state = None
    if isinstance(ckpt, dict) and ('model' in ckpt or 'state_dict' in ckpt):
        state = ckpt.get('model', ckpt.get('state_dict'))
    elif isinstance(ckpt, dict):
        # heuristics: dict이지만 키가 텐서들로 보이면 곧바로 state_dict로 간주
        state = ckpt
    else:
        state = ckpt

    # DataParallel/DDP 가중치의 'module.' 프리픽스 제거 :contentReference[oaicite:3]{index=3}
    if isinstance(state, dict) and any(k.startswith('module.') for k in state.keys()):
        state = {k.replace('module.', '', 1): v for k, v in state.items()}

    model.load_state_dict(state, strict=strict)  # 주: load_state_dict에는 dict 객체를 넘겨야 함 :contentReference[oaicite:4]{index=4}
...
def main():
    ...
    val_loader = torch.utils.data.DataLoader(
        val_dataset,   # [FIX] 원래 train_dataset이던 버그를 수정
        batch_size=config['batch_size'],
        shuffle=False,
        num_workers=config['num_workers'],
        drop_last=False,
    ...
    if config.get('resume'):
        ...
        if os.path.isfile(resume_path):
            start_epoch, best_iou, best_dice = load_ckpt(
                resume_path, model, optimizer, scheduler, strict=True
            )
    ...
    trigger = 0
    for epoch in range(start_epoch, config['epochs']):  # [RESUME] start_epoch부터
        ...
        # [RESUME] 매 epoch 상태 저장 (마지막 ckpt)
        save_ckpt(os.path.join(output_dir, exp_name, 'last.pth'),
                  model, optimizer, scheduler, epoch, best_iou, best_dice, config)
        ...
        if val_log['iou'] > best_iou:
            ...
            # [RESUME] 베스트 체크포인트도 함께 저장
            best_iou = val_log['iou']
            best_dice = val_log['dice']
            save_ckpt(os.path.join(output_dir, exp_name, 'best.pth'),
                      model, optimizer, scheduler, epoch, best_iou, best_dice, config)
